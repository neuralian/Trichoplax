...
# simulation parameters
nFrames = 150     # number of animation frames
...
predator_margin = 0
predator_speed =  0.25
predator_brownian = posterior_diffusion_coefficient
...
for n_likelihood_particles = N_LIKELIHOOD_PARTICLES
    for n_posterior_particles = N_POSTERIOR_PARTICLES
        for posteriorDeathRate = POSTERIOR_DEATH_RATE
            global trialNumber = trialNumber + 1
            for rep = 1:N_REPS
           ...
           # Vreceptor_RF(prey)
            ...
            #println(prey.observer.Pparticle)
       ...
       #     initialize_particles(prey) # draw initial sample from prior
            ...
            observation_plt = scatter!(left_panel, prey.observer.nLparticles[:],
            ...
            # nb this is a dummy plot
            # the correct particle locations are inserted before first plot
            belief_plt = scatter!(left_panel, prey.observer.nPparticles[:], 
            ...
            #for i in 1:nFrames                                         # simulate without writing video file
                ...
                if SHOW_ANIMATION
                    ...
                    observation_plt[1].val = prey.observer.Sparticle

                    # update observation particle plot
                    belief_plt[1].val = prey.observer.Bparticle
        ...
        #    # initialize_posterior(prey)     # initialize prior
        #     samplePrior(prey) # draw initial sample from prior           θ = π * rand()[] # Random initial heading (from above)
