...
mat = poly!(scene,
       ...
       color = RGBA(.42, .66, .14, .2), strokewidth = 0, strokecolor = :black)
...
prey = poly!(scene,
       ...
       strokewidth = .5, strokecolor = :black)
preyGut = poly!(scene,
      decompose(Point2f0, Circle(Point2f0(0.,0.), preyRadius-preyMargin)),
      color = RGBA(1., 0.85, 0.75, 1.0), facealpha = 0.1,
      strokewidth = 0.0, strokecolor = :black)
preyStep = [0.0, 0.0]
