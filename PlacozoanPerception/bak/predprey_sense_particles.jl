...
nLhd_particles = 2500   # number of particles in likelihood sample
...
clock =text!(scene,"t = 0.0s",textsize = 100, position = (-2250., -2250.))[end]
...
record(scene, "test.mp4", 1:Nframes) do i
    ...
    global predatorStep = 0.95*predatorStep +
          0.05*randn(2).*25.0  .-
          predatorLocation ./ (4.0*sqrt(sum(predatorLocation.^2)))
    global preyStep = 0.95*preyStep + 0.05*randn(2).*25.
  #  global preyLocation = preyLocation .+ preyStep
    ...
    likelihood()
    sample_likelihood()
    LparticlePlot[1]=x[Int.(Lparticle[:,1])]
    LparticlePlot[2]=x[Int.(Lparticle[:,2])]
