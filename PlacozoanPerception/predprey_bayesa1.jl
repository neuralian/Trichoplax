    ...
    for j in 1:nLparticles
      if ix==Lparticle[j,1]  # found matching x-coord
        if Int(round(PParticle[i][2] + matRadius))==Lparticle[j,2] #&y-coord
          ...
          PParticle[ireplace] = PParticle[i] + Point2f0(5.0*randn(2))
          #nCollide +=1
        end
      end
    end
...
function newCollision(PParticle, Lparticle)

  for i in 1:length(PParticle)
    for j in 1:length(Lparticle)
      #println(i, ", ", j, ", ", distance(PParticle[i], Lparticle[j]))
  ...
  for j in 1:nPosterior_particles
    d = distanceFromOrigin(PParticle[j])
      if d>matRadius # edge of mat is reflecting barrier
         PParticle[j] = matRadius.*PParticle[j]./(2.0f0*d)
...
record(scene, "test.mp4", framerate = 24, 1:Nframes) do i
  ...
  if i > 0.85*Nframes
    ...
    global posteriorStep = 0.95f0*posteriorStep +
        0.5f0*[Point2f0(randn(2)) for k in 1:nPosterior_particles].*predatorSpeed
    ...
    newCollision(PParticle, Lparticle)
