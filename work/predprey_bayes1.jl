...
Lparticle = fill(0, (nLhd_particles,2)) # grid coords of Likelihood particles
likelyColor = :orange

# Posterior density parameters
nPosterior_particles = 2500
PParticle = fill(0, (nPosterior_particles,2)) # grid locations
postColor = :magenta
...
# return indices of posterior particles that collide with likelihood particles
function collisionBayes(PParticle, Lparticle)
   nCollide = 0
   for i in 1:nPosterior_particles
      x_collide = findall(x->x==PParticle[i,1], Lparticle[:,1]) # same x-grid
      collide = findall(x->x==PParticle[2,2], Lparticle[x_collide,2])# same y
      nCollide = nCollide + length(collide)
    end
    return nCollide
end
...
function initialize_posterior()
  ...
  while nP < nPosterior_particles
    candidate = rand(1:Ngrid,2)  # random grid point
    d = sqrt(x[candidate[1]]^2 + y[candidate[2]]^2) # candidate distance from origin
...
xPosterior = x[Int.(PParticle[:,1])]  # convert grid indices to x-y coords
yPosterior = y[Int.(PParticle[:,2])]
# plot likelihood particles (sample points)
PParticlePlot = scatter!(xPosterior, yPosterior,
...
record(scene, "test.mp4", 1:Nframes) do i
    ...
    global posteriorStep = 0.95*posteriorStep +
          0.05*randn(nPosterior_particles,2).*predatorSpeed
    global PParticle += posteriorStep


    PParticlePlot[1] = x[Int.(round.(PParticle[:,1]))]           # update reflected sample plot
    PParticlePlot[2] = y[Int.(round.(PParticle[:,2]))]
