...
function reflectObservation(xLhdSample, yLhdSample)
  ...
  observationPlot[1] = r.*xLhdSample./R            # update reflected sample plot
  observationPlot[2] = r.*yLhdSample./R
end

function reflectBelief(xLhdSample, yLhdSample)
  R = sqrt.(xLhdSample.^2 + yLhdSample.^2)
  r = preyRadius .- preyMargin*(R.-preyRadius)./(matRadius-preyRadius)
  #return (r.*xLhdSample./R, r.*yLhdSample./R)
  observationPlot[1] = r.*xLhdSample./R            # update reflected sample plot
...
record(scene, "test.mp4", framerate = 8, 1:Nframes) do i
...
#    s = reflectObservation(xParticle, yParticle)  # reflect samples into margin
    reflectObservation(xx, yy)  # reflect samples into margin
    ...
    global posteriorStep = 0.95*posteriorStep +
          0.25*randn(nPosterior_particles,2).*predatorSpeed
